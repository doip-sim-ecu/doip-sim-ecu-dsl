...
class SimDslEcuTest {
    ...
    fun `test request matching bytearray`() {
        ...
        val ecu = SimDslEcu(data)
        data.requests.add(Request("TEST", byteArrayOf(0x00, 0x10, 0x20), null) { first = true })
        data.requests.add(Request("TEST2", byteArrayOf(0x00, 0x10), null) { second = true })
    ...
    fun `test request matching regex`() {
        ...
        val ecu = SimDslEcu(data)
        data.requests.add(Request("TEST", null, Regex("1020.*")) { first = true })
        data.requests.add(Request("TEST2", null, Regex("10.*")) { second = true })
    ...
    fun `test continue matching`() {
        ...
        val ecu = SimDslEcu(data)
        data.requests.add(Request("TEST", byteArrayOf(0x00, 0x10), null) { first = true; continueMatching() })
        data.requests.add(Request("TEST2", byteArrayOf(0x00, 0x10), null) { second = true })
    ...
    fun `test timer`() {
        ...
        data.requests.add(Request("TEST", byteArrayOf(0x3E, 0x00), null) {
            ack();
